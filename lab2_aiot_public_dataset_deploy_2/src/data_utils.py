...
def train_decision_tree_model(train_df: pd.DataFrame) -> Pipeline:
    """Train a Decision Tree baseline for comparison with Logistic Regression."""
    model = Pipeline(steps=[
        ("scaler", StandardScaler()),
...
def compare_models(train_df: pd.DataFrame, test_df: pd.DataFrame) -> Dict:
    """Train both models and return a side-by-side metric comparison table."""
    models = {
        "LogisticRegression": train_baseline_model(train_df),
        "DecisionTree(max_depth=6)": train_decision_tree_model(train_df),
    }
    comparison = {}
...
def decision_from_outputs(
    ...
    # Confidence: distance from decision boundary (0.5). <threshold = low confidence.
    confidence = abs(occupancy_probability - 0.5)
    ...
    if is_anomaly:
        ...
        decision = "UNCERTAIN_HOLD_CURRENT_STATE"
        command_hint = "NO_AUTO_CONTROL"
        safety_note = (
            f"Confidence thấp ({confidence:.2f} < {low_confidence_threshold}); "
            "giữ nguyên trạng thái hiện tại, không thay đổi tự động."
        )
