...
def batch_predict(payloads: List[TelemetryInput]):
    """Accept a list of telemetry readings and return predictions for all of them.
    Maximum 100 records per request.
    """
    if len(payloads) > 100:
        from fastapi import HTTPException
        raise HTTPException(status_code=400, detail="Maximum 100 records per batch request.")
